"""TX path for the RT5372 (RT5392): build TXINFO + TXWI + bulk-OUT (WIRE ONLY).
...
descriptor is verified against the kernel wire-format, not by ``verify_pcap``.
...
Family wire-format facts honored, all from the kernel: the trailing +4 USB end pad is
mandatory [SRC rt2800usb.c:440-451 ``roundup(skb_len, 4) + 4``] — the chip silently drops
the bulk-OUT without it; ``QSEL=2`` (EDCA — the only QSEL the data path accepts,
rt2800usb.c:422); ``TX_OP=HT_NONE`` so the chip skips the RTS/CTS handshake; ``WIV=1`` +
zeroed IV words so the inject isn't WEP-encrypted with a zero IV (the crypto engine is
disarmed at init). RT539x uses a 16-byte (4-word) TXWI — same as the RF30xx family.
"""
...
# 802.11 duration/ID (NAV) aireplay-ng stamps into its deauth template. The sequence
# number is stamped per-frame by the injector (driver.inject_frame), not here.
...
def build_deauth(target_mac: bytes, bssid: bytes, *, src_mac: bytes | None = None,
    ...
    """A 26-byte 802.11 deauth MPDU (no FCS), matching aireplay-ng's template:
    FC=0xC000, duration=0x013a, addr1=target / addr2=src / addr3=bssid, seqctl=0 (the
    injector stamps the running sequence number), reason."""
