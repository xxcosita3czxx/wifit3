"""RX path for the RT5372 (RT5392): bulk-IN URB → 802.11 MPDU + RSSI.
...
URB layout (one or more aggregated frames):
  ...
  * RXWI_W2          = signed per-path RSSI bytes (2T2R uses paths 0/1)
...
class Endpoints:
    ...
    def primary_bulk_in(self) -> int:
        if not self.bulk_in:
            raise RuntimeError("rt5372: no bulk-IN endpoint found")
...
def probe_endpoints(dev: usb.core.Device, *, interface: int = 0) -> Endpoints:
    ...
    logger.debug("rt5372 endpoints: bulk_in=%s bulk_out=%s",
