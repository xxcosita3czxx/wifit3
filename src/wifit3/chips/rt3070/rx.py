"""RX path for the RT3070: bulk-IN URB → 802.11 MPDU + RSSI.

Ported from ``rt2800usb_fill_rxdone`` (rt2800usb.c:481-577) + ``rt2800_process_rxwi``
/ ``rt2800_agc_to_rssi`` (rt2800lib.c:856-942). The bulk-IN stream is chip→host and
never enters the control gate, so this is verified on hardware (beacon-count A/B), not
by ``verify_pcap``; the descriptor offsets are re-derived from the kernel + rt2800usb.h.
...
URB layout (one or more aggregated frames):
  ...
  * RXWI_W2          = signed per-path RSSI bytes
...
flags it; remove it before trimming to MPDU_TOTAL_BYTE_COUNT or the last 2 body bytes
(an EAPOL M2 key-data tail) fall outside the window.
"""
...
class Endpoints:
    ...
    def primary_bulk_in(self) -> int:
        if not self.bulk_in:
            raise RuntimeError("rt3070: no bulk-IN endpoint found")
...
def probe_endpoints(dev: usb.core.Device, *, interface: int = 0) -> Endpoints:
    ...
    logger.debug("rt3070 endpoints: bulk_in=%s bulk_out=%s",
